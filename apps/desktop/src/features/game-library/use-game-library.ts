 ...
 * both. The connection card owns the sync state machine and its invalidation;
 * the ready-state controls can invoke that same action without duplicating
 * the query logic. Freshness comes from the API's `lastSyncedAt` and the
 * explicit sync action. Nothing here fetches on every render.
 */
